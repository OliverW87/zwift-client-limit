...
class Profile:
    ...
    @property
    def followers(self, start=0, limit=200):
        self.check_player_id()
        return self.request.json(
            '/api/profiles/{}/followers'.format(self.player_id))

    def get_followers(self, start=0, limit=200):
        self.check_player_id()
        return self.request.json(
            '/api/profiles/{}/followers?start={}&limit={}'.format(
    ...
    def get_followees(self, start=0, limit=200):
        self.check_player_id()
        return self.request.json(
            '/api/profiles/{}/followees?start={}&limit={}'.format(
                self.player_id, start, limit))
